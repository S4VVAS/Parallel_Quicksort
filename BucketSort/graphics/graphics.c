...
void DrawLine(float x1, float y1, float W, float H, float lHeight, float color){
 
    
    int x = (int)(x1/W*width);
    int y = (int)(y1/H*height);
    int y2 = (int)(y-lHeight);
    int icolor;
    ...
    XSetForeground(global_display_ptr, gc, colors[icolor]);
    XDrawLine(global_display_ptr, pixmap, gc, x, y, x, y2);
}
